...
p <- ggplot2::ggplot(plotData, ggplot2::aes(x = date, y = count)) +
      ...
      ggplot2::scale_y_continuous(name = "Monthly downloads", breaks = yBreaks, labels = yBreaks, 
      ...
      ggplot2::geom_curve(data = data.frame(x = as.Date(releases), y = rep(max(yBreaks) - 25, 
                          length(releases)), xend = as.Date(releases), yend = rep(max(yBreaks) - 100, length(releases))), 
                 ggplot2::aes(x = x, y = y, xend = xend, yend = yend), colour = "#555555", size=1, curvature = 0, 
...
bbplot::finalise_plot(plot_name = p, source = "Source: https://cran-logs.rstudio.com", 
                      save_filepath = paste0(dirname(getwd()), "/jfa/man/figures/downloads/downloads.png"),
                      width_pixels = 640, height_pixels = 350, 
                      logo_image_path = paste0(dirname(getwd()), "/jfa/man/figures/logo/jfaLogo.png"))
