...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(confidence, expected[rows], likelihood = "binomial", materiality = m[columns])) })
...
ktable <- knitr::kable(table, 
                       digits = 0, 
                       caption = "<center>Table 1: Statistical Sample Sizes based on the Binomial Distribution — 10 Percent Risk of Overreliance<br>(with number of expected errors in parentheses)</center>",
                       align = "l", 
                       format = "html", 
                       row.names = FALSE, 
...
ktable <- knitr::kable(table, 
                       digits = 0, 
                       caption = "<center>Table 2: Statistical Sample Sizes based on the Binomial Distribution — 5 Percent Risk of Overreliance<br>(with number of expected errors in parentheses)</center>",
                       align = "l", 
                       format = "html", 
                       row.names = FALSE, 
                       format.args = list(big.mark = ",", scientific = FALSE),
...
ktable <- knitr::kable(table, 
                       digits = 0, 
                       caption = "<center>Table 3: Statistical Sample Sizes based on the Binomial Distribution — 1 Percent Risk of Overreliance<br>(with number of expected errors in parentheses)</center>",
                       align = "l", 
                       format = "html", 
                       row.names = FALSE, 
                       format.args = list(big.mark = ",", scientific = FALSE),
