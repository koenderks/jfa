...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(confidence = 0.90, expectedError = expected[rows], likelihood = "binomial", materiality = m[columns])) }, silent = T)
...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(confidence = 0.95, expectedError = expected[rows], likelihood = "binomial", materiality = m[columns])) }, silent = T)
      if(class(p) == "try-error"){
        next 
      }
      tab[rows, columns] <- paste0(ss$sampleSize, " (", ss$expectedSampleError, ")")
...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(confidence = 0.975, expectedError = expected[rows], likelihood = "binomial", materiality = m[columns])) }, silent = T)
      if(class(p) == "try-error"){
        next 
      }
      tab[rows, columns] <- paste0(ss$sampleSize, " (", ss$expectedSampleError, ")")
...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(confidence = 0.99, expectedError = expected[rows], likelihood = "binomial", materiality = m[columns])) }, silent = T)
      if(class(p) == "try-error"){
        next 
      }
      tab[rows, columns] <- paste0(ss$sampleSize, " (", ss$expectedSampleError, ")")
