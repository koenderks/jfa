...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
      ss <- jfa::evaluation(conf.level = 0.90, n = n[rows], x = k[columns], materiality = 1, method = "poisson")
...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
      ss <- jfa::evaluation(conf.level = 0.95, n = n[rows], x = k[columns], materiality = 1, method = "poisson")
      tab[rows, columns] <- ceiling(ss$ub * 1000) / 10
  }
}
...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
      ss <- jfa::evaluation(conf.level = 0.975, n = n[rows], x = k[columns], materiality = 1, method = "poisson")
      tab[rows, columns] <- ceiling(ss$ub * 1000) / 10
  }
}
...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
      ss <- jfa::evaluation(conf.level = 0.99, n = n[rows], x = k[columns], materiality = 1, method = "poisson")
      tab[rows, columns] <- ceiling(ss$ub * 1000) / 10
  }
}
