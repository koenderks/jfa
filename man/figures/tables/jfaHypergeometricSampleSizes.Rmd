...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(confidence, expectedError = expected[rows], likelihood = "hypergeometric", materiality = m[columns], N = 100)) })
...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(confidence, expectedError = expected[rows], likelihood = "hypergeometric", materiality = m[columns], N = 500)) })
      if(class(p) == "try-error"){
        next 
      }
      tab[rows, columns] <- paste0(ss$sampleSize, " (", ss$expectedSampleError, ")")
...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(confidence, expectedError = expected[rows], likelihood = "hypergeometric", materiality = m[columns], N = 1000)) })
      if(class(p) == "try-error"){
        next 
      }
      tab[rows, columns] <- paste0(ss$sampleSize, " (", ss$expectedSampleError, ")")
