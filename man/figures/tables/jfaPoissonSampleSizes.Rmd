...
ktable <- knitr::kable(table, 
                       digits = 0, 
                       caption = "<center>Table 1: Statistical Sample Sizes based on the Poisson Distribution — 10 Percent Risk of Overreliance<br>(with expected sum of taints in parentheses)</center>",
                       align = "l", 
                       format = "html", 
                       row.names = FALSE, 
...
ktable <- knitr::kable(table, 
                       digits = 0, 
                       caption = "<center>Table 2: Statistical Sample Sizes based on the Poisson Distribution — 5 Percent Risk of Overreliance<br>(with expected sum of taints in parentheses)</center>",
                       align = "l", 
                       format = "html", 
                       row.names = FALSE, 
                       format.args = list(big.mark = ",", scientific = FALSE),
...
table <- as.data.frame(tab, stringsAsFactors = F)
table <- cbind(expected = paste0(round(expected * 100, 2), "%"), table)
colnames(table) <- c('Expected Errors', paste0(round(m * 100, 2), "%"))
table[is.na(table)] <- ""
...
ktable <- knitr::kable(table, 
                       digits = 0, 
                       caption = "<center>Table 3: Statistical Sample Sizes based on the Poisson Distribution — 1 Percent Risk of Overreliance<br>(with expected sum of taints in parentheses)</center>",
                       align = "l", 
                       format = "html", 
                       row.names = FALSE, 
                       format.args = list(big.mark = ",", scientific = FALSE),
