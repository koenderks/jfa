...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
      ss <- jfa::evaluation(confidence = 0.90, nSumstats = n[rows], kSumstats = k[columns], materiality = 1, method = "poisson")
...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
      ss <- jfa::evaluation(confidence = 0.95, nSumstats = n[rows], kSumstats = k[columns], materiality = 1, method = "poisson")
      tab[rows, columns] <- ceiling(ss$confBound * 1000) / 10
  }
}
...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
      ss <- jfa::evaluation(confidence = 0.975, nSumstats = n[rows], kSumstats = k[columns], materiality = 1, method = "poisson")
      tab[rows, columns] <- ceiling(ss$confBound * 1000) / 10
  }
}
...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
      ss <- jfa::evaluation(confidence = 0.99, nSumstats = n[rows], kSumstats = k[columns], materiality = 1, method = "poisson")
      tab[rows, columns] <- ceiling(ss$confBound * 1000) / 10
  }
}
