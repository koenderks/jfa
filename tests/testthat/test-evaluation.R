...
test_that(desc = "(id: f3-v0.6.5-t7) Test evaluation with non-conjugate priors", {
  ...
  expect_equal(res$mle, 0.004612450275)
  expect_equal(res$ub, 0.04719364302)
  set.seed(1)
  prior <- auditPrior(method = "param", likelihood = "uniform", alpha = 0, beta = 0.5)
  res <- evaluation(materiality = 0.03, x = 0, n = 53, prior = prior, method = "binomial")
  expect_equal(res$mle, 0.004349117104)
  expect_equal(res$ub, 0.05654508462)
  set.seed(1)
  prior <- auditPrior(method = "param", likelihood = "t", alpha = 0.01)
  res <- evaluation(materiality = 0.03, x = 0, n = 53, prior = prior, method = "binomial")
  expect_equal(res$mle, 0.003807214653)
  expect_equal(res$ub, 0.05189620594)
})
