context("Comparison to AuditSampler package")
...
test_that(desc = "Sample size tables for Hypergeometric distribution", {
  ...
  for(i in 1:nrow(sampleSizeMatrix)){
    jfaRes <- planning(materiality = sampleSizeMatrix[i, 3], 
                       confidence = sampleSizeMatrix[i, 4], 
                       expectedError = sampleSizeMatrix[i, 2], 
                       likelihood = "hypergeometric",
                       N = sampleSizeMatrix[i, 1])  
