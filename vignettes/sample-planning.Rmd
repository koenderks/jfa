...
this example the performance materiality is set to 5 percent of the total
population value, meaning that the population cannot contain more than 5
...
misstatement in the population is lower than 5 percent of the population value.
...
planning(
  materiality = 0.05, expected = 0, conf.level = 0.95,
...
In short, a multiple-stage sampling plan unfolds as follows. In the first stage
the auditor draws an initial sample of size $n_1$. If this sample contains a
tolerable number of misstatements (typically 0), then the auditor can
accept the population. However, if thes sample contains more than the tolerable
number of misstatements (e.g., 1), the population cannot be approved. In this
case, the sample is extended with another sample of size $n_2$. If this second
sample reveals a tolerable number of misstatements, then the population can be
accepted. If not, then the population should be rejected or a third sample $n_3$
...
instance, if the auditor wants to initially plan for 0 misstatements, but wants
to extend the sample if 1 misstatement is found, then the audit risk consists
of:

- $p(k = 0 | n_1)$: The probability of finding zero misstatements in the first
  sample of $n_1$ items; plus
- $p(k = 1 | n_1)$: The probability of finding one misstatement in the first
  sample of $n_1$ items; and
- $p(k = 0 | n_2)$: The probability finding one misstatement in the second sample
  of $n_2$ items.

The sum of these probabilities should be lower than the audit risk $\alpha$.

\begin{equation}
  p(k = 0 | n_1) + p(k = 1 | n_1) \cdot p(k = 0 | n_2) \leq \alpha
\end{equation}

If we choose the size of the extension equal to that of the initial
sample, $n_1$ = $n_2$ = $n$, we find that the sample size for a two-stage
sampling plan is the solution to the following equation.
...
probability of incorrectly rejecting the null hypothesis is below the audit risk
$\alpha$ = 0.05.
...
wants to extend the sample after finding three misstatements in the first stage,
extend further if they find one misstatement in the second stage, and approve
the population if they find zero misstatements in the third stage. The required
sample size per stage is 210.
...
rejecting the null hypothesis is below the audit risk $\alpha$ = 0.05.
...
The Bayesian approach to audit sampling does not require a large extension of
the sample size since it does not have the problems of optional stopping
[@rouder_2014].

```{r}
planning(materiality = 0.03, expected = 0, prior = TRUE)
```
