...
sample [@derks_2022]. For example, the command below creates an improper
...
as intolerable misstatement [@derks_2022] using `method = 'impartial'`. For
...
misstatement and the hypotheses of intolerable misstatement [@derks_2021]
...
has a probability of 60 percent into a beta prior distribution.
...
internal control risk) into a prior distribution [@derks_2021] using
...
to 90 percent and internal control risk is equal to 60 percent into a beta prior
distribution.
...
the information that the mode of the prior distribution is 1 percent and the
upper bound is 60 percent into a beta prior distribution.
...
distribution [@derks_2021] using `method = 'sample'` in combination with
...
[@derks_2021], into the prior distribution using `method = 'factor'` in
...
materiality of 5 percent, while using a beta(1, 10) prior distribution.
...
respect to a performance materiality of 5 percent after seeing a sample of 60
items with 1 misstatement, while using a normal(0.025, 0.05) prior distribution.
