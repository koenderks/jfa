...
Adding multiple prior distributions is done via the `+` operator, which computes a prior distribution containing the combined information from the prior distributions. For example, the information in `prior6` and `prior9` can be combined (added) via `prior6 + prior9`.

```{r}
prior10 <- prior6 + prior9
summary(prior10)
```
...
Weighing multiple prior distributions is done via the `*` operator, which computes the convolution (i.e., a weighted average under the assumption of independence) of the prior distributions. For example, the weighted average of `prior1` and `prior9` can be computed with `(a * prior1) * (b * prior9)`, where `a` and `b` are absolute weights. Note that the convolution assumes that the sum of two distributions of the same type (e.g., beta) is a new distribution of that type (e.g., beta), which is per definition an approximation of the true distribution.

```{r}
prior11 <- (0.5 * prior6) * (0.5 * prior9) # same as prior6 * prior9
summary(prior11)
```
...
```{r}
planning(materiality = materiality, expected = expected, conf.level = confidence, prior = prior10)
```
...
```{r}
evaluation(materiality = materiality, conf.level = confidence, x = 1, n = 60, prior = prior10)
```
