...
You can combine and weigh multiple prior distributions into a single prior using the `+` and `*` operators. For example, suppose you created two priors `prior1` and `prior9` and want to compute the weighted average (i.e., convolution) of the two distributions.

```{r}
prior10 <- 0.6 * prior1 + 0.4 * prior9
summary(prior10)
```
