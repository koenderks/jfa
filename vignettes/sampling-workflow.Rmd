...
Welcome to the 'Walkthrough of the classical audit sampling workflow' vignette
...
In a frequentist analysis applying the audit risk model, the auditor treats the
increased detection risk as their level of uncertainty. This implies that the
auditor promptly uses the adjusted confidence level `c.adj` to compute the
sample size via the `planning()` function.
...
The auditor can review the outcome of their planning procedure using the
`summary()` function. The results indicate that the auditor needs to select a
sample of $n$ = 178 items. This ensures that if a maximum of $k$ = 4.45
misstatements are found, they can assert with 91.66 percent confidence that the
misstatement in BuildIt's population is less than the performance materiality of
five percent.
...
misstatement in the population using the `evaluation()` function. 
...
The auditor can review the results of their inference using the `summary()`
function. The resulting upper bound is 1.396 percent with a confidence level of
91.66 percent, which is below the performance materiality of five percent.
...
Given that the 91.66 percent upper confidence bound on the misstatement in the
population is below the performance materiality, the auditor has gathered
sufficient evidence to conclude that the population does not contain material
misstatement.
