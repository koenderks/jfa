...
x <- evaluation(materiality = 0.05, data = allowances, values = "bookValue", values.audit = "auditValue", times = "times", prior = prior)
summary(x)
```
...
```{r}
x <- evaluation(materiality = 0.05, data = allowances, values = "bookValue", values.audit = "auditValue", strata = "branch", times = "times", alternative = "two.sided", N.units = as.numeric(table(allowances$branch)))
summary(x)
```
...
Bayesian inference can improve the stratum estimates by specifying a prior distribution that is then applied to each stratum and pooling information. The call below evaluates the `allowances` sample using a Bayesian multilevel stratified evaluation procedure.

```{r}
prior <- auditPrior(method = "arm", materiality = 0.05, ir = 0.5, cr = 0.5)
x <- evaluation(materiality = 0.05, data = allowances, values = "bookValue", values.audit = "auditValue", strata = "branch", times = "times", alternative = "two.sided", N.units = as.numeric(table(allowances$branch)), pooling = "partial", prior = prior)
summary(x)
```
