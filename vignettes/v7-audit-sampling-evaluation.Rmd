...
result_cp <- evaluation(
  ...
  n = sum(retailer$samples), x = sum(retailer$errors), N.units = sum(retailer$items),
...
```{r, echo = FALSE}
plot(1:20, rep(result_cp$mle, 20), pch = 19, ylim = c(0, 0.5), las = 1, main = "Stratum estimates", ylab = "Misstatement", xlab = "Stratum", axes = FALSE)
graphics::axis(side = 1, at = 1:20)
graphics::axis(side = 2, at = seq(0, 0.50, length.out = 6), las = 1)
graphics::arrows(x0 = 1:20, x1 = 1:20, y0 = rep(result_cp$lb, 20), y1 = rep(result_cp$ub, 20), angle = 90, length = 0.05)
graphics::arrows(x0 = 1:20, x1 = 1:20, y1 = rep(result_cp$lb, 20), y0 = rep(result_cp$ub, 20), angle = 90, length = 0.05)
```
...
result_pp <- evaluation(
  materiality = 0.05, method = "binomial", prior = TRUE, pool = TRUE,
  n = retailer$samples, x = retailer$errors, N.units = retailer$items,
  alternative = "two.sided"
)
summary(result_pp)
