...
This vignette outlines the most commonly used sample evaluation methodology for auditing and shows how to evaluate a sample using the `evaluation()` function in the **jfa** package. Evaluating an audit sample using the `evaluation()` function requires that the data from the sample is available in one of two forms:
...
By default, `evaluation()` performs estimation and provides a point estimate of the population misstatement and a (`conf.level` $\times$ 100)% confidence / credible interval around this estimate. However, in audit sampling there typically applies a maximum tolerable error to the population, the performance $\theta_{max}$. You can provide this critical value to the `evaluation()` function as a fraction using the `materiality` argument. Next to estimation which is performed by default, specifying `materiality` triggers the comparison of two competing hypotheses:

$$H_1:\theta<\theta_{max}\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\, H_0:\theta\geq\theta_{max}$$.

Evidence in favor or against the hypothesis of tolerable misstatement is then quantified using the *p*-value (classical) or the Bayes factor (Bayesian).
...
For this example, we take the `BuildIt` that set that comes with the package. This data set contains 3500 financial statement line items, each with a booked value `bookValue` and, for illustrative purposes, and audited (true) value `auditValue`. Since the focus of this vignette is the evaluation stage in the audit, the first 300 items are selected as an example. The performance materiality in this example is set to 5%.
...
The call below evaluates the `BuildIt` sample using a classical non-stratified evaluation procedure.
...
The call below evaluates the `BuildIt` sample using a Bayesian non-stratified evaluation procedure.
...
As an example, we add a stratum indicator to the `BuildIt` data set to indicate stratum membership of the line items. Here, each line item comes from one of three branches of the auditee.
...
The call below evaluates the `BuildIt` sample using a classical stratified evaluation procedure.

```{r}
x <- evaluation(materiality = 0.05, data = sample, values = "bookValue", values.audit = "auditValue", strata = "stratum", alternative = "two.sided", N.units = as.numeric(table(BuildIt$stratum)))
summary(x)
```

In this case, the output shows that the population estimate is 2.50% with a 95% confidence interval ranging from 1.22% to 4.96%. The stratum estimates can be seen in the output of the `summary()` function and are visualized below.
...
Bayesian inference can improve the stratum estimates by specifying a prior distribution that is then applied to each stratum. The call below evaluates the `BuildIt` sample using a Bayesian stratified evaluation procedure.

```{r}
prior <- auditPrior(method = "arm", materiality = 0.05, ir = 0.5, cr = 0.5)
x <- evaluation(materiality = 0.05, data = sample, values = "bookValue", values.audit = "auditValue", strata = "stratum", alternative = "two.sided", N.units = as.numeric(table(BuildIt$stratum)), prior = prior)
summary(x)
```
