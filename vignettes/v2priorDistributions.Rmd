...
```{r}
prior3 <- auditPrior(confidence, materiality, expectedError, method = 'bram', likelihood = likelihood, ub = 0.6)
prior3
```
...
```{r}
prior4 <- auditPrior(confidence, materiality, expectedError, method = 'median', likelihood = likelihood)
prior4
```
...
```{r}
prior5 <- auditPrior(confidence, materiality, expectedError, method = 'hypotheses', likelihood = likelihood, pHmin = 0.6)
prior5
```
...
```{r}
prior6 <- auditPrior(confidence, materiality, expectedError, likelihood = likelihood, method = 'sample', sampleN = 30, sampleK = 0)
prior6
```
...
```{r}
prior7 <- auditPrior(confidence, materiality, expectedError, method = 'factor', likelihood = likelihood, sampleN = 58, sampleK = 0, factor = 0.7)
prior7
```
