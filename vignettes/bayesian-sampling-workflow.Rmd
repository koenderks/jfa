...
`auditPrior()` function, she can create a prior distribution that incorporates
...
Now that the prior distribution is specified, Laura can calculate the required
sample size for her desired inference by using the `planning()` function. She
uses the `prior` object as input for the `planning()` function.
...
misstatements are found, they can conclude with 95% confidence that the
...
a 95% probability that the misstatement in BuildIt's population is lower than
2.278%.
...
Since the 95% upper credible bound on the misstatement in population is lower
than the performance materiality, the auditor has obtained sufficient evidence
to conclude that the population does not contain material misstatements. The
auditor can create a `html` or `pdf` report of the statistical results using
