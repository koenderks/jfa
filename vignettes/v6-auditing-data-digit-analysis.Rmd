...
Welcome to the 'Digit analysis' vignette of the `jfa` package. In this vignette you are able to find detailed examples of how you can incorporate the `data_digit_distribution()` and `data_repeated_values()` functions provided by the package.
...
x <- data_digit_distribution(sinoForest$value, check = "first", reference = "benford")
print(x)
plot(x)

# Bayesian hypothesis test using default prior
x <- data_digit_distribution(sinoForest$value, check = "first", reference = "benford", prior = TRUE)
print(x)
plot(x)
```
...
```{r}
x <- data_repeated_values(sanitizer$value, check = "lasttwo", B = 2000)
print(x)
plot(x)
```
