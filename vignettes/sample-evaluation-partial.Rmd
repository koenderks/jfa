...
of each item, which is defined as the proportional misstatement in the item: 
...
therefore does not take into account the abundance of zeros in the data.
...
misstatement [@broeze_2006, Chapter 4.4]. Despite its somewhat unconventional
...
In `jfa`, the hurdle beta model can be fitted using `method = "hurdle.beta"`. To
estimate the parameters in this model using maximum likelihood estimation, we
use the default option `prior = FALSE`.
...
object created by the `auditPrior()` function). In this case, the posterior
distribution cannot be determined analytically and must be determined using MCMC
sampling, which is why `set.seed(1)` is called.
