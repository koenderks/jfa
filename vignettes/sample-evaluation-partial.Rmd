...
`method = "binomial"` in the `evaluation()` function.
...
\begin{cases}
    ...
    \text{Beta}(\alpha, \beta) & \text{with probability}\, 1 - p
\end{cases}
$$

In `jfa`, the hurdle beta model can be fitted using `method = "hurdle.beta"` in
the `evaluation()` function. To estimate the parameters in this model using
maximum likelihood estimation, we use the default option `prior = FALSE`.
...
8.88 percent, with a 95 percent upper bound of around 14.44 percent. Since the
...
`method = "binomial"` in comination with `prior = TRUE` in the `evaluation()`
function. As usual, `prior` can also be an object created by the `auditPrior()`
function.
...
It is also possible to fit a Bayesian variant of the hurdle beta model. This can
be done using `method = "hurdle.beta"` in combination with `prior = TRUE` (or an
object created by the `auditPrior()` function) in the `evaluation()` function.
In this case, the posterior distribution cannot be determined analytically and
must be determined using MCMC sampling, which is why `set.seed(1)` is called.
