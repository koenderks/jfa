...
the African American, Asian, and Hispanic groups in comparison to the reference
group (Caucasian). See Pessach & Shmueli (2022) for a more detailed explanation
of these metrics.
...
1. **Demographic parity (Statistical parity)**: Compares the number of positive
  ...
  metric.
    ...
    The formula for the demographic parity metric is $DP = TP + FP$, and the
      parity ratio for sensitive group $i$ is given by
      $\frac{DP_{i}}{DP_{reference}}$.
  ...
  metric = "dp"
    ...
    **Interpretation:**
    - *African American*: The parity ratio for African Americans compared to
      ...
      African Americans predicted as reoffenders in these data by the algorithm
      than Caucasians.
    - *Asian*: The ratio for Asians is very close to zero (0.0059524),
      indicating that there are many less Asians (4) that are predicted as
      reoffenders in these data than there are Caucasians (672).
    - *Hispanic*: The ratio for Hispanics is 0.22173, meaning that there are
      about five times less Hispanics predicted as reoffenders in these data
      than that there are Caucasians.

` `

2. **Proportional parity (Disparate impact)**: Compares the proportion of
  positive predictions of each ethnic group with that in the reference group.
  For example, in the case that a positive prediction represents a reoffender,
  equal proportional parity requires the proportion of predicted reoffenders to
  be similar across groups.

    The formula for the proportional parity metric is 
      $PP = \frac{TP + FP}{TP + FP + TN + FN}$, and the parity ratio for
      sensitive group $i$ is given by $\frac{PP_{i}}{PP_{reference}}$.
  ...
  metric = "pp"
    ...
    **Interpretation:**
    - *African American*: The ratio of true positive rates (TPRs) for African
      Americans compared to Caucasians is 1.8521. This indicates that the TPR
      for African Americans is approximately 1.85 times higher than for
      ...
      indicating that the null hypothesis of equal proportional parity should be
      rejected.
    - *Asian*: The proportional parity ratio for Asians is 0.4038, indicating
      that their TPR is lower than for Caucasians. This suggests potential
      underestimation of reoffenders among Asians.
    - *Hispanic*: The ratio for Hispanics is 0.91609, suggesting that their TPR
      is close to the reference group (Caucasians). This indicates relatively
      fair treatment of Hispanics in the algorithm's predictions.

    This is probably also a good time to show the `summary()` and `plot()`
      functions associated with the `model_fairness()` function. Let's examine
      the frequentist function call again, but instead of printing the output to
      the console, this time we store the output in `x` and run the `summary()`
      and `plot()` functions on this object.
...
plot(x)
    ...
    For this (and all of the following) metrics, Bayesian inference is supported
      and provides credible intervals and Bayes factors. Like in the other
      functions in `jfa`, performing a Bayesian analysis using a default prior
      can be achieved by setting `prior = TRUE`. The resulting Bayes factors in
      favor of rejection of the null hypothesis of equality ($BF_{10}$) are
      shown in the output below. The prior and posterior distribution on the log
      odds ratio is displayed in the figure.
...
the reference group.
    ...
    The formula for the predictive rate parity metric is
      $PRP = \frac{TP}{TP + FP}$, and the parity ratio for sensitive group $i$
      is given by $\frac{PRP_{i}}{PRP_{reference}}$.
  ...
  metric = "prp"
    ...
    **Interpretation:**
    - *African American*: The predictive rate parity ratio for African Americans
      ...
      This indicates potential favoritism towards African Americans in the
      overall positive predictions made by the algorithm.
...
with the reference group.
    ...
    The formula for the accuracy parity metric is
      $AP = \frac{TP + TN}{TP + FP + TN + FN}$, and the parity ratio for
      sensitive group $i$ is given by $\frac{AP_{i}}{AP_{reference}}$.
...
model_fairness(
  ...
  metric = "ap"
    ...
    **Interpretation:**
    - African American: The accuracy parity ratio for African Americans is
...
group.
    ...
    The formula for the false negative rate parity metric is
      $FNRP = \frac{FN}{TP + FN}$, and the parity ratio for sensitive group $i$
      is given by $\frac{FNRP_{i}}{FNRP_{reference}}$.
...
non-reoffenders) of each ethnic group with the reference group.
    ...
    The formula for the false positive rate parity metric is
      $FPRP = \frac{FP}{TN + FP}$, and the parity ratio for sensitive group $i$
      is given by $\frac{FPRP_{i}}{FPRP_{reference}}$.
...
rates (e.g., for reoffenders) of each ethnic group with the reference group.
    ...
    The formula for the true positive rate parity metric is
      $TPRP = \frac{TP}{TP + FN}$, and the parity ratio for sensitive group $i$
      is given by $\frac{TPRP_{i}}{TPRP_{reference}}$.
...
(e.g., for non-reoffenders) of each ethnic group with the reference group.
    ...
    The formula for the negative predicted value parity metric is
      $NPVP = \frac{TN}{TN + FN}$, and the parity ratio for sensitive group $i$
      is given by $\frac{NPVP_{i}}{NPVP_{reference}}$.
  ...
  metric = "npvp"
    ...
    **Interpretation:**
    - *African American*: The Negative predicted value (NPV) parity ratio for
      African Americans is 0.98013. A value close to 1 indicates that the NPV
      for African Americans is very similar to the reference group (Caucasians).
      This suggests fair treatment in predicting non-reoffenders among African
      Americans.
...
(true negative rate) of each ethnic group with the reference group.
    ...
    The formula for the specificity parity metric is $SP = \frac{TN}{TN + FP}$,
      and the parity ratio for sensitive group $i$ is given by
      $\frac{SP_{i}}{SP_{reference}}$.
...
model_fairness(
  ...
  metric = "sp"
    ...
    **Interpretation:**
    ...
    - *Asian*: The specificity parity ratio for Asians is 1.1731, indicating
      ...
      among Asians.
    - *Hispanic*: The specificity parity ratio for Hispanics is 1.0399,
      suggesting that their specificity is very close to the reference group.
