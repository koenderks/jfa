...
explanation of some of these metrics.
...
model_fairness(
  ...
  metric = "dp"
    ...
    ***Interpretation:***
    - *African American*: The demographi parity for African Americans compared 
    ...
    - *Asian*: The demographic parity for Asians is very close to zero
      ...
      Naturally, this can be explained because of the lack of many Asian people
      (31) in the data.
...
2. **Proportional parity (Disparate impact)**: Compares the proportion of
  ...
  reoffender, equal proportional parity requires the proportion of predicted
  ...
  metric = "pp"
    ...
    ***Interpretation:***
    - *African American*: The proportional parity for African Americans compared
      ...
      indicating that the null hypothesis of equal proportions should be
      rejected.
    ...
    This is a good time to show the `summary()` and `plot()` functions
      ...
      frequentist function call again, but instead of printing the output to the
...
plot(x)
    ...
    For this (and all of the following) metrics, Bayesian inference is supported
      ...
      shown in the output below. The prior distribution can be
      adjusted by setting `prior = 1` (the default), or providing a number
      representing the prior concentration parameter
      (see `plot(x, type = "posterior")` for a visualization of the prior and
      posterior distribution on the log odds ratio, see the figure below).
...
(e.g., for non-reoffenders) of each unprivileged group with the privileged group.
...
model_fairness(
  ...
  metric = "npvp"
    ...
    ***Interpretation:***
    - *African American*: The Negative predicted value (NPV) parity for
      African Americans is 0.98013. A value close to 1 indicates that the NPV
      for African Americans is very similar to the privileged group
      (Caucasians). This suggests fair treatment in predicting non-reoffenders
      among African Americans.
    - *Asian*: The NPV parity for Asians is 1.1163, indicating that their
    ...
    - *Hispanic*: The NPV parity for Hispanics is 1.0326, suggesting that
...
(true negative rate) of each unprivileged group with the privileged class.
  ...
  metric = "sp"
    ...
    ***Interpretation:***
    ...
    - *Hispanic*: The specificity parity for Hispanics is 1.0399,
      suggesting that their specificity is very close to the privileged class.
