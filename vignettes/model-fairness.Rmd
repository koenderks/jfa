...
The formula for the demographic parity metric is $DP = TP + FP$, and the parity
ratio for sensitive group $i$ is given by $\frac{DP_{i}}{DP_{reference}}$.
...
positive predictions of each ethnic group with that in the reference group. For
example, in the case that a positive prediction represents a reoffender, equal
proportional parity requires the proportion of predicted reoffenders to be
similar across groups.

The formula for the proportional parity metric is 
$PP = \frac{TP + FP}{TP + FP + TN + FN}$, and the parity ratio for sensitive
group $i$ is given by $\frac{PP_{i}}{PP_{reference}}$.
...
This is probably also a good time to show the `summary()` and `plot()` functions
associated with the `model_fairness()` function. Let's examine the frequentist
function call again, but instead of printing the output to the console, this
time we store the output in `x` and run the `summary()` and `plot()` functions
on this object.
...
The formula for the predictive rate parity metric is $PRP = \frac{TP}{TP + FP}$,
and the parity ratio for sensitive group $i$ is given by
$\frac{PRP_{i}}{PRP_{reference}}$.
...
$AP = \frac{TP + TN}{TP + FP + TN + FN}$, and the parity ratio for sensitive
group $i$ is given by $\frac{AP_{i}}{AP_{reference}}$.
...
$FNRP = \frac{FN}{TP + FN}$, and the parity ratio for sensitive group $i$ is
given by $\frac{FNRP_{i}}{FNRP_{reference}}$.
...
$FPRP = \frac{FP}{TN + FP}$, and the parity ratio for sensitive group $i$ is
given by $\frac{FPRP_{i}}{FPRP_{reference}}$.
...
$TPRP = \frac{TP}{TP + FN}$, and the parity ratio for sensitive group $i$ is
given by $\frac{TPRP_{i}}{TPRP_{reference}}$.
...
$NPVP = \frac{TN}{TN + FN}$, and the parity ratio for sensitive group $i$ is
given by $\frac{NPVP_{i}}{NPVP_{reference}}$.
...
The formula for the specificity parity metric is $SP = \frac{TN}{TN + FP}$, and
the parity ratio for sensitive group $i$ is given by
