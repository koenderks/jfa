...
We can try out some examples with the `BuildIt` data set that is included in the package. It includes a population of 3500 items from a fictional construction company BuildIt, which we can use to illustrate statistical audit sampling using the `jfa` package (for more info, see ?BuildIt).
...
Because this data set contains the recorded values of the items we need to consider each monetary unit in the population as a possible unit of inference.
...
Selecting a sample using the `selection()` function requires that you have knowledge of the sampling units. Items can be selected using *record sampling* (also called attribute sampling) with `units = "records"`, or using *monetary unit sampling* with `units = "mus"`. 

It also requires knowledge of the sampling algorithm. Items can be selected using *random sampling* with `algorithm = "random"`, using *cell sampling* with `algorithm = "cell"`, or using fixed interval sampling (also known as systematic sampling) with `algorithm = "interval"`. 

### Record sampling

For example, the code below samples 60 items from the `BuildIt` data set using a *random record sampling* scheme.
...
After executing the audit and annotating the items in the sample with their audit values, you can evaluate whether you have achieved your analysis goal via the `evaluation()` function. The function can be used with summary statistics from the sample, or with an annotated sample as input. 
...
Suppose that in 60 items you have found 1 misstatement. Using `nSumstats = 60` and `kSumstats = 1` you can specify the outcomes of the sample in the `evaluation()` function. Do not forget to specify your analysis goal using the `materiality` or `minPrecision` arguments.
...
Suppose that you have audited the items in the sample and have found no deviations from the recorded values. 
