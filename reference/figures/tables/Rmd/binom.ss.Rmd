...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- planning(conf.level = 0.90, expected = expected[rows], likelihood = "binomial", materiality = m[columns])) }, silent = T)
...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- planning(conf.level = 0.95, expected = expected[rows], likelihood = "binomial", materiality = m[columns])) }, silent = T)
      if(inherits(p, "try-error")){
        next 
      }
      tab[rows, columns] <- paste0(ss$n, " (", ss$x, ")")
...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- planning(conf.level = 0.975, expected = expected[rows], likelihood = "binomial", materiality = m[columns])) }, silent = T)
      if(inherits(p, "try-error")){
        next 
      }
      tab[rows, columns] <- paste0(ss$n, " (", ss$x, ")")
...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- planning(conf.level = 0.99, expected = expected[rows], likelihood = "binomial", materiality = m[columns])) }, silent = T)
      if(inherits(p, "try-error")){
        next 
      }
      tab[rows, columns] <- paste0(ss$n, " (", ss$x, ")")
