...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(conf.level, expected = expected[rows], likelihood = "hypergeometric", materiality = m[columns], N.units = 100)) }, silent = T)
...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(conf.level, expected = expected[rows], likelihood = "hypergeometric", materiality = m[columns], N.units = 500)) }, silent = T)
      if(inherits(p, "try-error")){
        next 
      }
      tab[rows, columns] <- paste0(ss$n, " (", ss$x, ")")
...
for(rows in 1:length(expected)){
  for(columns in 1:length(m)){
    if(expected[rows] >= m[columns]){
      ...
      p <- try({ suppressMessages(ss <- jfa::planning(conf.level, expected = expected[rows], likelihood = "hypergeometric", materiality = m[columns], N.units = 1000)) }, silent = T)
      if(inherits(p, "try-error")){
        next 
      }
      tab[rows, columns] <- paste0(ss$n, " (", ss$x, ")")
