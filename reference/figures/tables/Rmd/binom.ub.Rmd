...
for (rows in 1:length(n)) {
  for (columns in 1:length(k)) {
    ss <- evaluation(conf.level = 0.90, n = n[rows], x = k[columns], materiality = 0.99, method = "binomial")
...
for (rows in 1:length(n)) {
  for (columns in 1:length(k)) {
    ss <- evaluation(conf.level = 0.95, n = n[rows], x = k[columns], materiality = 0.99, method = "binomial")
    tab[rows, columns] <- ceiling(ss$ub * 1000) / 10
  }
}
...
for (rows in 1:length(n)) {
  for (columns in 1:length(k)) {
    ss <- evaluation(conf.level = 0.975, n = n[rows], x = k[columns], materiality = 0.99, method = "binomial")
    tab[rows, columns] <- ceiling(ss$ub * 1000) / 10
  }
}
