...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
    if(n[rows] >= N)
      next
      tab[rows, columns] <- evaluation(materiality = 0.10, n = n[rows], x = k[columns], prior = auditPrior(method = 'median', materiality = 0.10, likelihood = 'hypergeometric', N.units = N))
  }
}
...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
    if(n[rows] >= N)
      next
      tab[rows, columns] <- evaluation(materiality = 0.05, n = n[rows], x = k[columns], prior = auditPrior(method = 'median', materiality = 0.05, likelihood = 'hypergeometric', N.units = N))
  }
}
...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
    if(n[rows] >= N)
      next
      tab[rows, columns] <- evaluation(materiality = 0.02, n = n[rows], x = k[columns], prior = auditPrior(method = 'median', materiality = 0.02, likelihood = 'hypergeometric', N.units = N))
  }
}
...
for(rows in 1:length(n)){
  for(columns in 1:length(k)){
    if(n[rows] >= N)
      next
      tab[rows, columns] <- evaluation(materiality = 0.01, n = n[rows], x = k[columns], prior = auditPrior(method = 'median', materiality = 0.01, likelihood = 'hypergeometric', N.units = N))
  }
}
