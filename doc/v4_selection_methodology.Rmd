---
title: 'Selection: Sampling methodology'
author: Koen Derks
...
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Selection: Sampling methodology}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteDepends{jfa}
  %\VignetteKeywords{algorithm, audit, methodology, sampling, selection}
  %\VignettePackage{jfa}
...
This section discusses the three sampling algorithms implemented in `jfa`. First, for notation, let the set $S$ of all possible samples $s_i$ be defined as:

$$S = \{s_1, s_2,\dots,s_M\},$$

and define the population $N$ as the total set of sampling units $x_j$.

$$N = \{x_1, x_2, \dots, x_N\}.$$

Theoretically, every sample $s_i$ is a set of sampling units and is assigned a selection probability $p(s_i)$ with which this set occurs. Practically, this is equivalent to each sampling unit $x_j$ in the population having a selection probability $p(x_j)$. The purpose of the sampling algorithm is to assign selection probabilities to each of the sampling units, and draw sampling units from the population until a set of size $n$ has been created.
...
where $N$ is the number of units in the population. To clarify this procedure, Figure 1 provides an illustration of the simple random sampling algorithm.

<center>

![Figure 1: Illustration of random sampling](img/randomSampling.png)
...
As an example, the code below shows how to apply the random sampling algorithm in a record sampling and a monetary unit sampling setting.
...
with the property that the space between selected units $i$ is the same as the interval $I$, see Figure 2. However, in practice the selection is deterministic and completely depends on the chosen `intervalStartingPoint`.

<center>

![Figure 2: Illustration of fixed interval sampling](img/fixedIntervalSampling.png)

</center>

The fixed interval algorithm, like the random sampling algorithm, yields a sample that allows every sampling unit in the population an equal chance of being selected. However, the systematic sampling algorithm has the property that all items in the population with a monetary value larger than the interval $I$ have an selection probability of one because one of these items' sampling units are always selected from the interval. Note that, if the population is arranged randomly with respect to its deviation pattern, fixed interval sampling is equivalent to random selection.
...
As an example, the code below shows how to apply the fixed interval sampling algorithm in a record sampling and a monetary unit sampling setting. Note that, by default, the first sampling unit from each interval is selected. However, this can be changed by setting the argument `intervalStartingPoint = 1` to a different value.
...
![Figure 3: Illustration of cell sampling](img/cellSampling.png)
...
As an example, the code below shows how to apply the cell sampling algorithm in a record sampling and a monetary unit sampling setting.
