...
.partial_pooling <- function(method, prior.x, prior.n, n.obs, t.obs, t, nstrata, stratum, likelihood) {
  ...
  data <- switch(likelihood,
    "binomial" = list(S = nstrata - 1, n = n.obs[-1], k = t.obs[-1], priorx = prior.x, priorn = prior.n),
    "beta" = list(S = nstrata - 1, "t" = (t[[1]] * (n.obs[1] - 1) + 0.5) / n.obs[1], "n" = n.obs[1], "s" = as.numeric(stratum), priorx = prior.x, priorn = prior.n)
  )
  suppressWarnings({
    utils::capture.output(
      file = "NUL",
      raw <- rstan::sampling(
        object = stanmodels[[paste0(likelihood, "_", method)]], data = data, chains = 4, cores = 1, verbose = FALSE, pars = "theta_s",
