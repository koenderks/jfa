...
Below you can find a explanation of the functions in the current version
of `jfa`, sorted by their occurrence in the standard audit sampling
workflow.
...
the inherent risk and control risk in the audit risk model. The returned
object is of class `jfaPrior` and can be used with associated `print()`
and `plot()` methods. `jfaPrior` results can also be used as input
argument for the `prior` argument in other functions.
